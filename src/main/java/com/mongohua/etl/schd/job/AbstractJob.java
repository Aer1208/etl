...
public abstract class AbstractJob extends InitJdbc implements Job {
    ...
    private synchronized void triggerEvent(int jobId, long instId, String vDate) {
        ...
        try {
            ...
            List<Map<String, Object>> events = jdbcTemplate.queryForList("select a.job_id,a.ref_type,a.ref_job_id,a.week_offset " +
                    "  from t_job_ref a " +
                    "  join t_job_def b on a.job_id=b.job_id " +
                    " where a.ref_job_id=? " +
                    "   AND a.ref_type!=1" +
            ...
            for (Map<String, Object> event : events) {
                ...
                int weekOffset = 0;
                if (event.containsKey("week_offset") && !StringUtils.isEmpty(event.get("week_offset"))) {
                    try {
                        weekOffset = Integer.parseInt(event.get("week_offset").toString());
                    } catch (Exception e) {
                ...
                if ("2".equals(refType)) {
                    ...
                    if (trigger.getCycleUnit() == 1 && refJob.getCycleUnit() == 0 && 24 % refJob.getJobCycle() == 0) {
                        generateRefCnt(triggerId, refJobId, vDate.substring(0, 8), refType, 24, refJob.getJobCycle(), vDate);
                    }
                    ...
                    if (trigger.getCycleUnit() == 2 && refJob.getCycleUnit() == 1 && refJob.getJobCycle() == 1) {
                        ...
                        if (totCnt == -1) {
                            ...
                            generateRefCnt(triggerId, refJobId, vDate.substring(0, 6), refType, totCnt, 1, vDate);
                        }
                    }
                    ...
                    if (trigger.getCycleUnit() == 3 && refJob.getCycleUnit() == 2 && refJob.getJobCycle() == 1) {
                        generateRefCnt(triggerId, refJobId, vDate.substring(0, 4), refType, 12, 1, vDate);
                    }
                } else if ("5".equals(refType)) { // 周作业依赖日作业
                    if (trigger.getCycleUnit() == 5 && refJob.getCycleUnit() == 1 && refJob.getJobCycle() == 1) {
                        String endWeekDate = Util.getEndWeekDay(vDate, weekOffset);
                        generateRefCnt(triggerId, refJobId, endWeekDate, refType, 7, 1, vDate);
                    }
                } else if ("6".equals(refType)) { // 季度作业依赖月作业
                    if (trigger.getCycleUnit() == 6 && refJob.getCycleUnit() == 3 && refJob.getJobCycle() == 1) {
                        String endMonthOfQuarter = Util.getEndMonthOfQuarter(vDate);
                        generateRefCnt(triggerId, refJobId, endMonthOfQuarter, refType, 3, 1, vDate);
                    }
                }
            }
     ...
     */
    private void generateRefCnt(String triggerId, String refJobId, String vDate, String refType, int totCnt, int jobCycle, String triggerDate) {
        ...
        if (eventCnt == 0) {
            ...
            jdbcTemplate.update("insert into t_job_ref_cnt(job_id, ref_job_id, ref_type,data_date, tot_cnt, succ_cnt)" +
                    "values(?,?,?,?,?,?)", triggerId, refJobId, refType, vDate, totCnt, jobCycle);
        } else {

            int refJobSuccCnt = jdbcTemplate.queryForObject
                    ("select count(*) from t_job_inst where job_id=? and data_date=? and status=1", new Object[]{refJobId, triggerDate}, Integer.class);

            if (refJobSuccCnt == 1) { // 只有第一次执行成功，才计数
                // 否则更新，对完成的计数+ 依赖作业周期数
                ...
                logger.warn("触发作业重复运行，跳过计算， 触发作业：{}，被触发作业:{}, 触发日期：{} ", refJobId, triggerId, triggerDate);
            }
        }
    }
