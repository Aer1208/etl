var TOOL_BAR = [ {
    ...
    handler : function() {
        ...
        if(selectedArray.length==1) {
            ...
            $("#refType").combobox("setValue",selectedArray[0].refType);
...
$(function() {
    ...
    $("#refType").combobox(refTypeOption)
    ...
    $('#save').click(function() {
        ...
        var refType = $("#refType").combobox("getValue");
    ...
    $("#tt").datagrid( {
        ...
        columns:[[
            ...
            {field:'refType',title:'依赖类型', width:80,align:'center',formatter:function (val,index) {
                if (val == 1) return "直接依赖"
                else if (val == 2) return "天依赖小时"
                else if (val == 3) return "月依赖天"
                else if (val == 4) return "年依赖月"
             }}
        ]]
    });
});
